...
indexcomp<- as.data.frame(matrix(data = NA, nrow = 1, ncol = 17, byrow = FALSE, dimnames = NULL))

names(indexcomp) <- c("site", "season", "SpeciesCode", "english_name", "scientific_name", "analysis_code", "sort", "BandCensuscor1", "sig1", "DETBCcor2", "sig2", "Censussig", "Bandingsig", "R2", "BCsig", "BCR2") 
...
indexcomp$Censussig<-coef(summary(lm))[3,4]
indexcomp$Bandingsig<-coef(summary(lm))[2,4]
indexcomp$R2<-summary(lm)$r.squared
}
...
try(indexcomp$Bandingsig<-coef(summary(lm2))[2,4], silent=TRUE)
try(indexcomp$R2<-summary(lm2)$r.squared, silent=TRUE)
}
...
try(indexcomp$Censussig<-coef(summary(lm1))[2,4], silent=TRUE)
try(indexcomp$R2<-summary(lm1)$r.squared, silent=TRUE)
}
...
tab <- read_csv("output/ Annual Index Comparision.csv")
tab <- tab %>% select(-SpeciesCode, - X15)
...
write.table(tab, file = paste(out.dir,"Annual Index Comparision_Sig.csv"), row.names = FALSE, col.name = TRUE, append = TRUE, quote = FALSE, sep = ",")
